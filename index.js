...
import LineEquation from './lib/LineEquation';
import * as MathHelper from './lib/MathHelper';
...
export default class PerspectiveGrid {
  ...
  update() {
    ...
    let topLine = MathHelper.getLineParams(this._tl.x, this._tl.y, this._tr.x, this._tr.y);
    let bottomLine = MathHelper.getLineParams(this._bl.x, this._bl.y, this._br.x, this._br.y);
    let leftLine = MathHelper.getLineParams(this._tl.x, this._tl.y, this._bl.x, this._bl.y);
    let rightLine = MathHelper.getLineParams(this._tr.x, this._tr.y, this._br.x, this._br.y);
  ...
  getCenterAt(column, row) {
    ...
    const center = MathHelper.getMassCenter.apply(null, this.getQuadAt(column, row));
  ...
  drawSquares() {
    ...
    for (let i = 0; i < points.length; i++) {
      ...
      const p1 = this.horizontal[point.y - 1].intersect(this.vertical[point.x - 1]);
  ...
  _getVanishingEquation(side, oppositeSide) {
    ...
    if (side === null && oppositeSide === null) {
      ...
      vanishing = new Point(
        this._tl.x,
        MathHelper.getVerticalConvergence(this._tl.x, oppositeSide.m, oppositeSide.c)
      );
  ...
  _getHorizon(vVanishing, hVanishing) {
    ...
    const horizon = MathHelper.getLineParams(vVanishing.x, vVanishing.y, hVanishing.x, hVanishing.y);
